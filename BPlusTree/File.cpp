...
FilePointer::FilePointer( const char * filename, lone int offset, int nDupKeys ) {
...
// TEST only 
FilePointer::FilePointer( const char * filename, lone int offset, int nDupKeys, string word ) {
  this->filename = strdup( filename );
  this->offset = offset;
  this->word = word;
}
